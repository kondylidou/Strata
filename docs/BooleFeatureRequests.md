...
- **`decreases` annotation on functions, procedures, and `for` loops**
  ...
  - Remaining gap (→ #1167): `decreases <int expr>` closes the language gap; int-recursive functions are pure UFs in SMT — manual axioms still needed for functional properties.
...
- **`Sequence T` type and slicing ops**
  ...
  - Remaining gap: recursive spec functions over sequences need int-based termination proofs (→ #1167).
...
11. **Sequence slicing**: Implemented. Remaining gap: int-based termination for recursive seq functions (blocks B1–B5) → #1167.
...
17. **Mutual recursion / forward references**: Implemented for functions over datatypes (structural recursion via `@[cases]`). Remaining gap: mutual recursion over `int` or other non-datatype types — blocked by int-based termination (same as Gap #11, → #1167).
...
| [`widening_casts.lean`](../StrataTest/Languages/Boole/FeatureRequests/widening_casts.lean) | Widening casts in quantifiers/comparisons | Verus `guide/integers`, `quantifiers`, `statements` | Updated to use `e as_int` (Gap #6); map-lookup case `v[i] as_int` verified |
| [`cast_expr.lean`](../StrataTest/Languages/Boole/FeatureRequests/cast_expr.lean) | `e as_int` — widening cast, all bitvector widths | dalek-lite `scalar.rs` B2/B5 (`u8/u64/u128 as nat`) | Gap #6 closed; bv1/8/16/32/64/128 |
...
| [`seq_slicing.lean`](../StrataTest/Languages/Boole/FeatureRequests/seq_slicing.lean) | Sequence slicing (`subrange`, `skip`, `take`, `drop_first`) and all 8 Core `Sequence.*` ops | dalek-lite `scalar_specs.rs`, `core_specs.rs`; Vest `leb128`, `repetition` | Implemented (#1075); remaining gap: recursive spec functions over sequences (Gap #11, → #1167) — language gap closed by #1167, pure UF in SMT; `reconstruct` commented in seed pending merge |
| [`scalar_reduce.lean`](../StrataTest/Languages/Boole/FeatureRequests/scalar_reduce.lean) | `reduce()` spec axiom for B2 (`Scalar::from_bytes_mod_order_wide`) | dalek-lite `scalar.rs` | Implemented (#1075); `u8_64_as_group_canonical` (Gap #11, → #1167): recursive form commented in seed; pure UF in SMT once merged, manual axioms unchanged |

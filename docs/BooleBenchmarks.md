...
| 5 | Operator trait-method dispatch | ✓ resolved | `mul_clamped`'s `&self * &s` call dispatches to the resolved impl `Impl__13_mul` — the parser reads VLIR's `resolved_method`, guarded by `getDeclAny?` to emitted in-crate impls so external `Deref`/`Clone`/`Iterator::next` stay abstract (no dangling call). `mul_clamped`'s correctness postcondition `Impl__14_mul_clamped_ensures` now verifies against the impl's real Montgomery ensures; cvc5 **239/329** at a 3s cap (89 ⌛, 1 enc err). The lone encoding error is the now-orphaned abstract `Ops_Arith_Mul_mul`'s generic ensures (`Unimplemented encoding for type var`), removable by pruning the unreferenced global. See [`b5_minimal.lean`](../StrataBoole/StrataBooleTest/FeatureRequests/b5_minimal.lean) |
